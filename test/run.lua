-- test/run.lua
-- Test runner for headless Neovim (luaunit-based)
...
local function get_test_files()
  ...
  if pattern:match('^test/') or pattern:match('^test\\') then
    ...
    -- e.g., "example" -> test/**/*example*_spec.lua
    files = vim.split(vim.fn.globpath('test', string.format('**/*%s*_spec.lua', pattern)), '\n')

    -- Also try exact match: e.g., "example_spec.lua"
    if #files == 0 then
      files = vim.split(vim.fn.globpath('test', string.format('%s*_spec.lua', pattern)), '\n')
    end
...
local temp_pattern = '/tmp/tasks_nvim_test_'
